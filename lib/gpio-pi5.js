...
// Resolve which gpiochip carries the 40-pin header (the RP1 controller,
// labelled "pinctrl-rp1"). Resolution order:
//   1. GPIO_CHIP env var, if set  — explicit override always wins.
//   2. Auto-detect: the chip whose label is "pinctrl-rp1".
//   3. Fallback to 0.
// The chip number is NOT stable across distros/kernels: on Ubuntu 24.04 the
// header is typically gpiochip4, whereas a Raspberry Pi OS kernel with the
// alias patch puts it on gpiochip0. Auto-detection makes the plugin work on
// both without configuration. Resolution is lazy + memoised so it runs once,
// after GPIO permissions are in effect (avoids an EACCES at require time).
const RP1_LABEL = 'pinctrl-rp1';
let _resolvedChip; // undefined until first resolution
 
function resolveChip() {
	if (_resolvedChip !== undefined) return _resolvedChip;
 
	const env = process.env.GPIO_CHIP;
	if (env !== undefined && env !== '') {
		const n = Number(env);
		_resolvedChip = Number.isInteger(n) ? n : env; // number or chip name
		return _resolvedChip;
	}
 
	try {
		const names = libgpiod.chipNames || [];
		for (const name of names) {
			try {
				const c = new Chip(name);
				if (c.label === RP1_LABEL) {
					const m = /(\d+)$/.exec(c.name);
					_resolvedChip = m ? Number(m[1]) : name;
					return _resolvedChip;
				}
			} catch (e) {
				/* chip not openable; skip */
			}
		}
	} catch (e) {
		/* chipNames unavailable (e.g. permissions); fall through to default */
	}
...
class Gpio {
	...
	constructor(pin, direction, edge, options) {
		...
		this.chipId = this.options.chip !== undefined ? this.options.chip : resolveChip();
